...
private:
    ...
    std::unique_ptr<StructuredProduct> createProduct(
        const PricingInputs& inputs) const;
    void loadSettings();
...
PricerWindow::PricerWindow(QWidget* parent) : QWidget(parent) {
    ...
    timesEdit_ = new QLineEdit(QString::fromStdString(
        vectorToString(defaults_.observationTimes)));
    ...
    // Scroll wrapper so tall configurations (e.g., Heston + cliquet params) stay usable.
...
std::size_t PricerWindow::readSizeT(QLineEdit* edit, std::size_t fallback) const {
    const QString text = edit->text().trimmed();
    if (text.isEmpty()) {
        return fallback;
    }
...
unsigned int PricerWindow::readUInt(QLineEdit* edit, unsigned int fallback) const {
    const QString text = edit->text().trimmed();
    if (text.isEmpty()) {
        return fallback;
    }
...
PricingInputs PricerWindow::gatherInputs() const {
    ...
    if (inputs.productFamily == ProductFamily::Autocall &&
        inputs.autocallType == AutocallType::StepDown) {
        inputs.callBarriers =
            parseTimesList(callBarriersEdit_->text().trimmed().toStdString(),
                           inputs.callBarriers);
    }
    if (inputs.productFamily == ProductFamily::Autocall &&
        inputs.autocallType == AutocallType::Airbag) {
    ...
    if (inputs.productFamily == ProductFamily::Cliquet) {
        ...
        inputs.cliquetCap =
            readDouble(cliquetCapEdit_, defaults_.cliquetCap);
    }
    ...
    inputs.observationTimes =
        parseTimesList(timesEdit_->text().trimmed().toStdString(),
                       defaults_.observationTimes);
...
// Show/hide groups based on selected family/type and refresh layout/scroll area.
...
std::unique_ptr<StructuredProduct> PricerWindow::createProduct(
    const PricingInputs& inputs) const {
    if (inputs.productFamily == ProductFamily::Autocall) {
        ...
        case CliquetType::MaxReturn:
            return std::make_unique<CliquetMaxReturn>(
                inputs.underlying, inputs.observationTimes, inputs.spot,
                inputs.notional);
    ...
    try {
        ...
        if (inputs.productFamily == ProductFamily::Autocall) {
            ...
            if (inputs.autocallType == AutocallType::StepDown) {
                ...
                addMarker(inputs.autocallBarrier, "Call barrier",
                          QColor("#1565c0"));
            }

            if (inputs.autocallType == AutocallType::Phoenix ||
                inputs.autocallType == AutocallType::MemoryPhoenix) {
                addMarker(inputs.couponBarrier, "Coupon barrier",
                          QColor("#2e7d32"));
            }
        }
